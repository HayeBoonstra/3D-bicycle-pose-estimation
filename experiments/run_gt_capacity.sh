#!/usr/bin/env bash
# Train GT-2D capacity ablations (S/B/L/X) sequentially on a server.
#
# Usage:
#   DATA_ROOT=/mnt/SmallSSD/.../posemamba_training_sequences ./experiments/run_gt_capacity.sh
#   ./experiments/run_gt_capacity.sh capacity_l_gt
#
# Prerequisite:
#   ./experiments/build_gt_training_corpus.sh
#   python experiments/make_gt_capacity_configs.py
#
# Checkpoints land in ${CHECKPOINT_BASE}/capacity_{s,b,l,x}_gt/
set -euo pipefail
...
MANIFEST="${CONFIGS_DIR}/gt_capacity_experiments.json"
...
if [[ ! -f "${MANIFEST}" ]]; then
  echo "error: missing ${MANIFEST}. Run: python experiments/make_gt_capacity_configs.py" >&2
  exit 1
fi

if [[ ! -d "${DATA_ROOT}/${GT_CORPUS}/BICYCLE/train" ]]; then
  echo "error: GT training corpus not found at ${DATA_ROOT}/${GT_CORPUS}" >&2
  echo "  Run: ./experiments/build_gt_training_corpus.sh" >&2
  exit 1
fi
...
echo "[done] all GT capacity runs finished (or skipped)"
