...
def evaluate(args, model_pos, test_loader, datareader):
    ...
    with torch.no_grad():
        for batch_idx, (batch_input, batch_gt) in enumerate(tqdm(test_loader)):
            if getattr(args, "max_eval_batches", 0) and batch_idx >= int(args.max_eval_batches):
                break
    ...
    if datareader is None:
        gt_all = np.concatenate(gts_all)
        pred = results_all - results_all[:, :, 0:1, :]
        gt = gt_all - gt_all[:, :, 0:1, :]
        e1 = float(np.mean(mpjpe(pred, gt)))
...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    ...
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):    
        if getattr(args, "max_batches", 0) and idx >= int(args.max_batches):
            break
        ...
        if has_3d:
            ...
            weights = torch.ones((predicted_3d_pos.shape[2],), device=predicted_3d_pos.device, dtype=predicted_3d_pos.dtype)
            w_mpjpe = weights
            loss_3d_w = weighted_mpjpe(predicted_3d_pos, batch_gt, w_mpjpe) # 3D weighted
            
            # Temporal Consistency Loss
            dif_seq = predicted_3d_pos[:,1:,:,:] - predicted_3d_pos[:,:-1,:,:]
            weights_joints = torch.ones_like(dif_seq, device=dif_seq.device)
            weights_mul = w_mpjpe
...
def train_with_config(args, opts):
    ...
    datareader = None
    dt_path = os.path.join(args.data_root, args.dt_file) if hasattr(args, "dt_file") else ""
