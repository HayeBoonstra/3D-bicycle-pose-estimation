...
_io_path = _REPO_ROOT / "3d_keypoint_detector_training" / "posemamba_bicycle_io.py"
_spec = importlib.util.spec_from_file_location("posemamba_bicycle_io", _io_path)
_mod = importlib.util.module_from_spec(_spec)
assert _spec.loader is not None
# Required on Python 3.8: dataclasses need the module registered before exec_module.
sys.modules[_spec.name] = _mod
_spec.loader.exec_module(_mod)
apply_bicycle_2d_noise = _mod.apply_bicycle_2d_noise

BICYCLE_SUBSET = "BICYCLE"


def _is_bicycle_subset(subset_list) -> bool:
    return BICYCLE_SUBSET in subset_list
...
class MotionDataset3D(MotionDataset):
    def __init__(self, args, subset_list, data_split):
        ...
        self.bicycle_2d_noise_sigma = float(getattr(args, "bicycle_2d_noise_sigma", 0.0))
        self.bicycle_image_2d = _is_bicycle_subset(subset_list)
    ...
    def __getitem__(self, index):
        ...
        if self.bicycle_image_2d:
            if motion_file.get("data_input") is None:
                raise ValueError(f"BICYCLE sample missing data_input: {file_path}")
            motion_2d = np.asarray(motion_file["data_input"], dtype=np.float32)
            if self.data_split == "train":
                if self.bicycle_2d_noise_sigma > 0:
                    motion_2d = apply_bicycle_2d_noise(motion_2d, sigma=self.bicycle_2d_noise_sigma)
