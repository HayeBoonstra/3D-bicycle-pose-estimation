...
class CrossScan_plus_poselimbs(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        ...
        ctx.shape = (B, C, H, W)
        xs = x.new_empty((B, 4, C, H * W))
        # Parent-style accumulation; defaults to identity for unknown joint layouts.
        indices = [0, 0, 1, 0, 3, 6, 3, 6, 10, 10, 4, 10, 10, 15, 15, 4, 0, 0]
        if W != len(indices):
...
class SelectiveScanMamba(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        if "selective_scan_cuda" not in globals():
            ctx.fallback = True
            return u
        ctx.fallback = False
...
class SelectiveScanCore(torch.autograd.Function):
    ...
    @torch.cuda.amp.custom_fwd
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        if "selective_scan_cuda_core" not in globals():
            ctx.fallback = True
            return u
        ctx.fallback = False
        ctx.delta_softplus = delta_softplus
...
class SelectiveScanOflex(torch.autograd.Function):
    ...
    @torch.cuda.amp.custom_fwd
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        if "selective_scan_cuda_oflex" not in globals():
            ctx.fallback = True
            return u
        ctx.fallback = False
        ctx.delta_softplus = delta_softplus
